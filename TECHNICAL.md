...
4. [Training Pipeline](#4-training-pipeline) — `taxi/1_train_model.py`
5. [Evaluation](#5-evaluation) — `taxi/2_evaluate_model.py`
6. [Streaming Application](#6-streaming-application) — `taxi/3_streaming_app.py`
...
Detection logic is implemented as functions in `taxi/3_streaming_app.py`: `load_model_artifacts()` and `detect_anomalies()`.
...
Implemented in `taxi/1_train_model.py`.
...
Implemented in `taxi/2_evaluate_model.py`.
...
Implemented in `taxi/3_streaming_app.py`. Orchestrates Kafka consumption, Spark processing, and Dash visualization.
...
|-----------|-------|--------|
| `hidden_dim` | 64 | Grid search (`taxi/4_grid_sweep.py --mode hyperparams`) |
| `num_layers` | 1 | Grid search |
| `dropout` | 0.2 | Grid search |
| `learning_rate` | 5e-4 | Grid search |
| `threshold_percentile` | 99.99% | Grid search |
| `train_weeks` | 8 | Split optimization (`taxi/4_grid_sweep.py --mode split`) |
...
Detection is implemented as functions in `taxi/3_streaming_app.py`:
