...
│
├── taxi/                                    # Numbered scripts -- the canonical workflow
...
> **Note:** `taxi/3_streaming_app.py` is intentionally skipped here. It is the Docker entrypoint for the visual streaming demo and is **not** meant to be run directly with `python`. See the [Docker demo with visual application](#docker-demo-with-visual-application) section below.

#### 2a. Train the baseline

```bash
uv run python taxi/1_train_model.py
```
...
```bash
uv run python taxi/2_evaluate_model.py
```
...
```bash
uv run python taxi/4_grid_sweep.py
```
...
```bash
uv run python taxi/2_evaluate_model.py --model-dir models/best
```
...
`taxi/3_streaming_app.py` is the Docker entrypoint for the live Kafka -> Spark -> Dash demo. It loads a trained model, consumes the NYC taxi CSV through Kafka, scores each weekly window in Spark Structured Streaming, and renders the results in a live Dash dashboard. **Do not run it directly with `python`** -- launch the full stack with Docker Compose:
...
The numbered files in `taxi/` tell the full reproduction story:
...
and flagged as anomalous when `a(w)` exceeds a threshold set at the 99.99th percentile of validation Mahalanobis distances. This is the same scoring scheme demonstrated cell-by-cell in `notebooks/model_design.ipynb` and is what `taxi/1_train_model.py` and `taxi/2_evaluate_model.py` reproduce end-to-end.
